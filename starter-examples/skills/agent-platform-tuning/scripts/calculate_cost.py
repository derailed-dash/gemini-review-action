...
def count_characters(input_file: str) -> int:
    ...
    if not input_file.startswith("gs://") and "://" in input_file:
        raise ValueError(f"Unsupported file path: {input_file}. Only local paths and gs:// paths are supported.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate tuning cost for a given dataset and model.")
    ...
    if args.model not in MODEL_DATA or args.tuning_mode not in MODEL_DATA[args.model]:
        print(f"Error: Tuning mode {args.tuning_mode} not supported for model {args.model}")
        sys.exit(1)
