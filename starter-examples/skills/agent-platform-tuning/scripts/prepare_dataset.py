...
def _format_row(row, format_type: str, prompt_col: str, completion_col: str) -> dict[str, Any]:
...
def convert_to_jsonl(
    ...
    try:
        if input_file.endswith(".csv"):
            ...
            dataset = datasets.load_dataset("parquet", data_files=input_file, split="train")
        else:
    ...
    if len(dataset) < initial_len:
        logging.warning("Dropped %d rows with empty or NaN values", initial_len - len(dataset))
    ...
    if validation_split and validation_split > 0:
        ...
        split_dict = formatted_dataset.train_test_split(seed=42, test_size=validation_split)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Prepare or Validate dataset for Agent Platform Model Tuning")
    parser.add_argument(
        "--input",
    ...
    if args.validate_only:
        ...
        if not all([args.input, args.prompt_col, args.completion_col]):
            logging.error("--input, --prompt_col, and --completion_col are required for conversion")
            sys.exit(1)
        convert_to_jsonl(
            args.input,
            args.output,
