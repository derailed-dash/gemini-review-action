...
def update_skill(args):
    ...
    if not update_mask_parts:
        print("Error: must provide at least one field to update (--display-name, --description, --zip-file, --folder)")
        sys.exit(1)
...
def list_skill_revision(args):
    ...
    endpoint = get_endpoint(args.location)
    url = (
        f"https://{endpoint}/v1beta1/projects/{args.project}/locations/{args.location}/skills/{args.skill_id}/revisions"
    )
