                ...
                rows.append(
                    {
                        ...
                        "metric_name": name,
                        "score": score,
                        "explanation": ((r.get("explanation") or "")[:200] if isinstance(r, dict) else ""),
                    }
                )
    return rows
...
def main():
    parser = argparse.ArgumentParser(
        description=("Print summary metrics and per-case scores from an Agent Platform Eval result JSON file.")
    )
    parser.add_argument(
        "--result",
        "-r",
