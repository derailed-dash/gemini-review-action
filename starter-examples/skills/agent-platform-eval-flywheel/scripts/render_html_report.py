...
def main():
    parser = argparse.ArgumentParser(description="Render Agent Platform Eval HTML reports from saved JSON artifacts.")
    parser.add_argument(
        "--input",
        "-i",
        required=True,
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "evaluation = client.evals.evaluate result; loss-analysis = client.evals.generate_loss_clusters response."
        ),
    )
    parser.add_argument(
        "--output",
    ...
    try:
        ...
        print(
            f"ERROR: requires the agentplatform SDK to be installed (`pip install google-cloud-aiplatform`): {e}",
            file=sys.stderr,
        )
        sys.exit(1)
