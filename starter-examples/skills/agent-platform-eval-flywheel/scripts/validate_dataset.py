...
def _validate_content(content: Any, path: str) -> list[ValidationError]:
    ...
    if not isinstance(content, dict):
        errors.append(ValidationError(path, f"Expected dict, got {type(content).__name__}"))
        return errors
...
def _validate_agent_event(event: Any, path: str) -> list[ValidationError]:
    ...
    if not isinstance(event, dict):
        errors.append(ValidationError(path, f"Expected dict, got {type(event).__name__}"))
        return errors
...
def _validate_turn(turn: Any, path: str) -> list[ValidationError]:
    ...
    if not isinstance(turn, dict):
        errors.append(ValidationError(path, f"Expected dict, got {type(turn).__name__}"))
        return errors
...
def _validate_agent_data(agent_data: Any, path: str) -> list[ValidationError]:
    ...
    if not isinstance(agent_data, dict):
        errors.append(ValidationError(path, f"Expected dict, got {type(agent_data).__name__}"))
        return errors
...
def _validate_eval_case(case: Any, index: int, metrics: list[str] | None) -> list[ValidationError]:
    """Validate a single EvalCase."""
    path = f"eval_cases[{index}]"
    errors = []

    if not isinstance(case, dict):
        errors.append(ValidationError(path, f"Expected dict, got {type(case).__name__}"))
        return errors
        ...
        for raw_metric in metrics:
            ...
            if metric in _MULTI_TURN_METRICS and not has_agent_data:
                errors.append(
                    ValidationError(
                        path,
                        f"Metric '{metric}' requires 'agent_data' with conversation turns.",
                    )
                )
...
def validate_dataset(dataset: dict[str, Any], metrics: list[str] | None = None) -> list[ValidationError]:
    """Validate an entire EvaluationDataset."""
    errors = []

    if not isinstance(dataset, dict):
        errors.append(ValidationError("root", f"Expected dict, got {type(dataset).__name__}"))
        return errors
...
def main():
    parser = argparse.ArgumentParser(description="Validate an evaluation dataset for Agent Platform Eval SDK.")
    parser.add_argument(
        "--dataset",
        "-d",
        required=True,
