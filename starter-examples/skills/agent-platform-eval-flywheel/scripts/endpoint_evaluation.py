    ...
    try:
        with requests.post(
            ...
            try:
                ...
                raise ValueError(f"Unable to parse prediction response: {json_response}")
            except (KeyError, IndexError, TypeError) as e:
                raise ValueError(f"Unable to parse prediction response: {json_response}") from e
...
def run_evaluation(
    *,
    ...
    df["response"] = df["prompt"].apply(lambda prompt: run_inference(prompt, endpoint_url, token))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("Evaluate an Agent Platform endpoint using Agent Platform Evaluation.")
    )
